...
export const Header = () => {
  ...
  const [darkMode, setDarkMode] = useState(
    JSON.parse(localStorage.getItem("darkValue")) || false
  );
  ...
  useEffect(() => {
    ...
    localStorage.setItem("darkValue", JSON.stringify(darkMode));
  }, [darkMode]);

  return (
    <header className=" ">
      <nav className="  bg-white border-gray-200 dark:bg-gray-900 border-b-2 dark:border-b-1 dark:border-gray-900 ">
        <div className="max-w-screen-2xl flex flex-wrap items-center justify-between lg:mx-5 p-4 -mx-3">
          <div className="flex items-center space-x-3 rtl:space-x-reverse  ">
            <Link to="/">
              <img
                src={Logo}
                className="h-10 lg:w-20 rounded "
                alt="Cinemate Logo"
              />
            </Link>
            <Link to="/">
          ...
          <div className="flex md:order-2" id="mobile device">
            <button
